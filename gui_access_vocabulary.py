...
class PrintVocabulary(tk.Toplevel):
    def __init__(self):
        ...
        row_num = 2
        global all_levels
        all_levels = flashcards.vocabulary.make_all_levels()
        for idx, level in enumerate(all_levels):
            create_title(f"Level {idx+1}", row_num)
            row_num += 1
            for item in level:
                create_vocabulary_entry(item, row_num)
                row_num += 1
        ...
        clicked_search_bar = search_entry.bind("<Button-1>", clear_search_bar)
        search_entry.bind("<Return>", search_vocabulary)
...
def create_title(level_num, row_num):
    tk.Label(master=frame,
             ...
             height=1,
             bg=logo_chip,
             relief=tk.SOLID).grid(columnspan=5, row=row_num, column=0, pady=1)


def create_vocabulary_entry(item, row_num):
    delete_btn = tk.Button(
        master=frame,
        image=delete_icon)
    delete_btn.image = delete_icon
    delete_btn.bind("<Double-Button-1>", delete_vocabulary)
    ...
    term_lbl.bind("<Double-Button-1>", make_changes)
...
def make_changes(event):
    ...
    changes_entry.bind("<Return>", submit_change)
...
def submit_change(event):
    ...
    if flashcards.vocabulary.check_if_vocab_already_exists(
            ...
            for item in level:
                if widget_column == 1:
                    if item.term == text_to_change:
                        ...
                        submit_change_to_testing_vocabulary(
        ...
        changed_text_lbl = tk.Label(
            ...
            height=1,
            font=("Poppins", 12))
        changed_text_lbl.bind("<Double-Button-1>", make_changes)
        changed_text_lbl.grid(row=widget_row, column=widget_column)


def submit_change_to_testing_vocabulary(text_to_change, changed_text):
    ...
    for level in all_levels:
        for item in level:
            if item.term == term_to_be_deleted["text"] \
                    and item.translation == translation_to_be_deleted["text"]:
                level.remove(item)
